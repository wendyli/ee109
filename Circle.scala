...
object Circle extends SpatialApp {
  ...
  def convolveVideoStream(): Unit = {
    ...
    Accel{
      val cirRad = SRAM[Int](maxCircles)
      ...
      Sequential{ // instantiate values
        ...
        Foreach(0 until cirCount){_=> // arbitrary number of loops, TODO: ideal would be a do while loop  
          Sequential.Foreach(0 until cirCount, 0 until cirCount){ (i,j) => 
            Pipe{
               val sqrRad = 4*cirRad(j)*cirRad(i)
        ...
        if(state == 0.to[Int]){ // Determine collision type  
            Sequential{
              ...
              Sequential.Foreach(0 until cirCount){ i =>  // detect border collision 
                Pipe{
                  borderCollision(i) = mux(cirX(i) + cirRad(i) >= Cmax || cirX(i) - cirRad(i) <= 0.to[Int] || cirY(i) + cirRad(i) >= Rmax || cirY(i) - cirRad(i) <= 0.to[Int], 1.to[Int], 0.to[Int])
              ...
              Sequential.Foreach(0 until cirCount, 0 until cirCount){ (i,j) =>  // detect ball to ball collision
                Pipe{
                  val sqrRad = 4*cirRad(i)*cirRad(j)
                  val distSqr = (cirX(i) - cirX(j)) * (cirX(i) - cirX(j)) + (cirY(i) - cirY(j))*(cirY(i) - cirY(j))
            ...
            Sequential{
              Sequential.Foreach(0 until cirCount){ i => 
                Pipe{
                  cirX(i) = mux( cirX(i) + cirVelX(i) > Cmax -cirRad(i), Cmax - cirRad(i), 
                            mux( cirX(i) + cirVelX(i) <= cirRad(i), cirRad(i), 
                                 cirX(i) + cirVelX(i)))

                  cirY(i) = mux( cirY(i) + cirVelY(i) > Rmax -cirRad(i), Rmax - cirRad(i), 
                            mux( cirY(i) + cirVelY(i) <= cirRad(i), cirRad(i),     
                                 cirY(i) + cirVelY(i)))
                }
              }
            }
              ...
              Foreach(0 until dwell){_=>
                Foreach(0 until Rmax, 0 until Cmax){ (r, c) =>
                  ...
                  Pipe{
                    ...
                    Sequential.Foreach(0 until cirCount){ i=>
                      Pipe{
                        val pixel = mux((r.to[Int64] - cirY(i).to[Int64])*(r.to[Int64] -cirY(i).to[Int64]) + (c.to[Int64] - cirX(i).to[Int64])*(c.to[Int64] -cirX(i).to[Int64]) < cirRad(i).to[Int64] * cirRad(i).to[Int64], 63.to[UInt6], 0.to[UInt6])
